...
xdata = n.arange(min(F_n[:,4])*0.999999,max(F_n[:,4])*1.000001,1e-8)
...
if do == 1:
	...
	mean_y = n.mean(exp_y)
	offset_exp_y = exp_y-mean_y
	spec_sum = n.zeros((len(exp_x_conv),))
	spec_int = 0
	amp=1
	for spec in n.arange(0,n_tr,1):#remake the spectrum, ready for comparison
		...
		#gamma_stim_used = 1/(n.pi*2*(1/gamma_e))*hbar*2.0*n.pi*n.sqrt(1.0+I_s/F_n[spec,8])
		boltz_FWHM = F_n[spec,4] * (8.0*kB_J*T*n.log(2)/(m_is_kg*c**2))**(0.5)
		boltz_HWHM_rel = (1.0/16)*(2.0*kB_J*T*n.log(2)/(m_is_kg*c**2))**(0.5)
	...
	spec_sum_norm = spec_sum/n.max(spec_sum)
	mult = n.max(offset_exp_y)
	spec_sum_plot = spec_sum_norm*mult+mean_y
	uncert = n.sqrt(exp_y)
	...
	chi_2_red = 1.0/len(exp_x_conv)*n.sum((spec_sum_plot-exp_y)**2/uncert**2)
	plt.clf()
	plt.errorbar(exp_x_conv,exp_y,uncert,fmt='k.',label=('Rb-87 Exp.'))
	plt.plot(exp_x_conv,spec_sum_plot,'-r',label=('Rb-87 Sim.'),)
	plt.legend()
	plt.xlabel('Energy (eV)')
	plt.ylabel('Photon Counts')
	plt.ticklabel_format(style='sci', axis='x', scilimits=(0,0))
	plt.minorticks_on()
	plt.text(n.mean(exp_x_conv),((n.min(spec_sum_plot)-n.min(uncert)+n.max(spec_sum_plot)+2.0*n.max(uncert)))/2.0,'$\chi^2_{red}$=%f \n Power = %.1f $\pm$ %.2f $\mu W$'%(chi_2_red,n.round(float(vec[12]/10e-3),decimals=1),0.01))
	#plt.text(n.mean(exp_x_conv),n.mean(exp_y)+100,'Power = %f $\mu W$'%(vec[12]/10e-3))
	plt.ylim((n.min(spec_sum_plot)-n.min(uncert),n.max(spec_sum_plot)+2.0*n.max(uncert)))
	#plt.savefig('Rb-87-vs-sim.png')
	print chi_2_red	
